...
def fitness(populatie, domeniu, precizie):
    valori_fitness = []
    for cromozom in populatie:
        x = decodeaza(cromozom, domeniu, precizie)
        valori_fitness.append(functie(x))
    return valori_fitness

def selectie(populatie, fit):
...
def algoritm_genetic(dimensiune, domeniu, precizie, a, b, c, prob_crossover, prob_mutatie, etape):
    ...
    max_fitness_evolutie = [] # evolutia in fiecare etapa
    
    with open("Evolutie.txt", "w") as f:
        for etapa in range(etape):
            fit = fitness(populatie, domeniu, precizie) # fitness pentru fiecare cromozom
            populatie, probabilitati, probabilitati_cumulate = selectie(populatie, fit)
            ...
            if etapa == 0:
                ...
                for i, c in enumerate(populatie):
                    x = decodeaza(c, domeniu, precizie)
                    f.write(f"Individ {i+1}: Cromozom={format(c, '020b')} X={x:.6f} f(X)={functie(x)}\n")
...
evolutie_max = algoritm_genetic(DIMENSIUNE_POPULATIE, DOMENIU, PRECIZIE, A, B, C, PROB_CROSSOVER, PROB_MUTATIE, NR_ETAPE)

print(max(evolutie_max))