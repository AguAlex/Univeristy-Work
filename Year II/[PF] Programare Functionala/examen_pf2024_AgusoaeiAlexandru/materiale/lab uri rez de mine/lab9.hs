...
-- instance ArbInfo Tree where
--     -- Inălțimea arborelui
--     level Empty = 0  -- Arborele gol are înălțimea 0
--     level (Node _ left middle right) = 1 + maximum [level left, level middle, level right]

--     -- Suma valorilor din arbore
--     sumval Empty = 0  -- Arborele gol nu are valoare
--     sumval (Node value left middle right) = value + sumval left + sumval middle + sumval right

--     -- Numărul de frunze din arbore
--     nrFrunze Empty = 0  -- Arborele gol nu are frunze
--     nrFrunze (Node _ Empty Empty Empty) = 1  -- Un nod cu 3 subarbori goi este o frunză
