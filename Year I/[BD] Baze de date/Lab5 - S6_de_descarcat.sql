...
1. Sa se creeze tabelul ANGAJATI_AAG (pnu se alcatuieste din prima litera din prenume 
si primele doua din numele studentului) corespunzator schemei relationale:
ANGAJATI_AAG(cod_ang number(4), nume varchar2(20), prenume varchar2(20), 
...
DROP TABLE ANGAJATI_AAG;

a) fara precizarea vreunei chei sau constrangeri;

create table ANGAJATI_AAG(
  cod_ang number(4), 
  nume varchar2(20), 
  prenume varchar2(20), 
  email char(15), 
  ...
  cod_sef number(4), 
  salariu number(8, 2), 
  cod_dep number(2)); --table ANGAJATI_AAG created.
  
  select * from angajati_AAG; --0 rez
  drop table angajati_AAG; --table ANGAJATI_AAG dropped.
...
create table ANGAJATI_AAG(
  cod_ang number(4), 
  nume varchar2(20) constraint null_nume_aag not null, 
  prenume varchar2(20), 
  email char(15), 
  data_ang date default sysdate, 
  job varchar2(10), 
  cod_sef number(4), 
  salariu number(8, 2) constraint null_salariu_aag not null, 
  cod_dep number(2),
  constraint pk_ang_aag primary key(cod_ang)
  );

select * from angajati_AAG;
    
SELECT constraint_name, constraint_type, table_name
FROM     user_constraints
WHERE  lower(table_name) IN ('angajati_aag','angajati_da');
--SYS_C00527493	C	ANGAJATI_AAG
--SYS_C00527492	C	ANGAJATI_AAG
--SYS_C00527494	P	ANGAJATI_AAG  
-- drop table angajati_AAG;
...
Se presupune ca valoarea implicita a coloanei data_ang este SYSDATE;
  ...
  SELECT constraint_name, constraint_type, table_name
FROM     user_constraints
WHERE  lower(table_name) ='angajati_AAG';
...
--NULL_SALARIU_AAG	C	ANGAJATI_AAG
--NULL_NUME_AAG	C	ANGAJATI_AAG
--PK_ANG_AAG	P	ANGAJATI_AAG

2.	Adaugati urmatoarele inregistrari in tabelul ANGAJATI_AAG:
...
insert into angajati_AAG(Cod_ang,Nume,Prenume,Email,Data_ang,Job,Cod_sef,Salariu,Cod_dep) 
values( 100	,'Nume1',	'Prenume1',	Null	,Null,	'Director',	null,	20000	,10);
select * from angajati_AAG;

insert into angajati_AAG
...
insert into angajati_AAG(Cod_ang,Nume,Prenume,Email,Data_ang,Job,Cod_sef,Salariu,Cod_dep) 
values( 102	,'Nume3',	'Prenume3',	'Nume3@gmail.com',  to_date('05-06-2004', 'dd-mm-yyyy'),
	'Analist',	101,5000,20);
insert into angajati_AAG(Cod_ang,Nume,Prenume,Email,Data_ang,Job,Cod_sef,Salariu,Cod_dep) 
values( 103	,'Nume4',	'Prenume4',	null,  null,
	'Inginer',	100,9000,20);
insert into angajati_AAG(Cod_ang,Nume,Prenume,Email,Data_ang,Job,Cod_sef,Salariu,Cod_dep) 
values( 104	,'Nume5',	'Prenume5',	'Nume5@gmail.com',  null,
	'Analist',	101,3000,30);

select * from angajati_AAG;

--truncate table angajati_AAG;
--rollback;
--select * from angajati_AAG;
--
--delete from angajati_AAG;

3. Creati tabelul ANGAJATI10_AAG, prin copierea angajatilor din departamentul 10 
din tabelul ANGAJATI_AAG. Listati structura noului tabel. Ce se observa?

create table angajati10_AAG as
  Select * from angajati_AAG where cod_dep =10;
  
  select * from angajati10_AAG; --2 rez
  
  
SELECT constraint_name, constraint_type, table_name
FROM     user_constraints
WHERE  lower(table_name) IN ('angajati10_AAG');
--SYS_C00527544	C	ANGAJATI10_AAG
--SYS_C00527543	C	ANGAJATI10_AAG
...
4. Introduceti coloana comision in tabelul ANGAJATI_AAG. 
Coloana va avea tipul de date NUMBER(4,2).

alter table angajati_AAG
...
--Table ANGAJATI_AAG altered.

--apoi pot sa micsorez?
alter table angajati_AAG
modify (comision number(6,2));
--table ANGAJATI_AAG altered. (toate infomatiiile din coloana comision sunt nule)

select * from angajati_AAG;
5-2 .Este posibil? modificarea tipului coloanei salariu in NUMBER(6,2)?
-- SALARIU  NOT NULL NUMBER(8,2) 
alter table angajati_AAG
modify (salariu number(8,2) default 1111);
...
insert into angajati_AAG (Cod_ang, Nume, Prenume, Job, Cod_sef, Cod_dep) 
values (105, 'Nume6', 'Prenume6', 'Inginer', 100, 20);

insert into angajati_AAG (Cod_ang, Nume, Prenume, Job, Cod_sef, Cod_dep, salariu)
...
12. Redenumiti tabelul ANGAJATI_AAG in ANGAJATI3_AAG.

--de rezolvat 
rename angajati_AAG to angajati3_AAG;

-- angajati_AAG TO succeeded.
select * from angajati_AAG; -- ORA-00942: tabelul sau vizualizarea nu exista
select * from angajati3_AAG; 
desc angajati3_AAG;

SELECT constraint_name, constraint_type, table_name
FROM     user_constraints
WHERE  lower(table_name) IN ('angajati3_AAG');


13. Consultati vizualizarea TAB din dictionarul datelor. 
Redenumiti angajati3_AAG in angajati_AAG.

select * from tab;
rename angajati3_AAG to angajati_AAG;
--Table renamed.
select * from angajati_AAG;
select * from angajati3_AAG;  --ORA-00942: tabelul sau vizualizarea nu exista

14. Suprimati continutul tabelului angajati10_AAG, fara a suprima structura acestuia.

create table angajati10_AAG as
  Select * from angajati_AAG where cod_dep =10;
  
  select * from angajati10_AAG;
  
  delete from angajati10_AAG; --tabela goala
  rollback; -- avem cele 2 inregistrari
  
  truncate table angajati10_AAG; --se pastreaza structura tabelei (DELETE+COMMIT)
  rollback; -- in continuare tabela este goala
  
  drop table angajati10_AAG;
  select * from angajati10_AAG; -- ORA-00942: tabelul sau vizualizarea nu exista
  
-- 15. Creati tabelul DEPARTAMENTE_AAG, corespunzator schemei relationale:
-- DEPARTAMENTE_AAG (cod_dep# number(2), nume varchar2(15), cod_director number(4))
-- specificand doar constrangerea NOT NULL pentru nume
-- (nu precizati deocamdata constrangerea de cheie primara). 

drop table departamente_AAG;

CREATE TABLE departamente_AAG 
(cod_dep number(2), 
nume varchar2(15) constraint NL_nume_AAG NOT NULL, 
cod_director number(4)); 

DESC departamente_AAG;
...
alter table departamente_AAG
add constraint pk_depart_AAG primary key (cod_dep); 

-- SQL Error: ORA-02437: (GRUPA32.PK_DEPART_AAG) nu a putut fi validata - cheia primara a fost violata
...
-- atentie
alter table departamente_AAG  --PK = Unique + NOT NULL
add constraint pk_depart_AAG primary key (cod_director);
...
--table DEPARTAMENTE_AAG altered.

--alter table departamente_AAG
--drop constraint pk_depart_AAG;
...
alter table angajati_AAG
add constraint fk_cod_dep foreign key(cod_dep) references departamente_AAG(cod_dep);
...
--ORA-02298: cannot validate (GRUPA34.FK_ANG_DEPART_AAG) - parent keys not found
--02298. 00000 - "cannot validate (%s.%s) - parent keys not found"
-- angajatii lucreaza pe departamente care nu se gasesc in tabela departamente_AAG
--insert  into angajati_AAG
...
NULL_SALARIU_AAG	C	ANGAJATI_AAG
NULL_NUME_AAG	C	ANGAJATI_AAG
FK_ANG_DEPART_AAG	R	ANGAJATI_AAG --FK
PK_ANG_AAG	P	ANGAJATI_AAG  --PK
...
--drop table departamente_AAG; --SQL Error: ORA-02449: cheile unice/primare din tabela sunt referite de cheile externe
...
SELECT constraint_name, constraint_type, table_name
FROM     user_constraints
WHERE  lower(table_name) = 'departamente_AAG';
...
SELECT constraint_name, constraint_type, table_name
FROM     user_constraints
WHERE  lower(table_name) IN ('angajati_AAG');

CK_SAL_COM_AAG	C	ANGAJATI_AAG
CK_COD_DEP_AAG	C	ANGAJATI_AAG
NULL_NUME_AAG	C	ANGAJATI_AAG
FK_ANG_DEPART_AAG	R	ANGAJATI_AAG
FK_ANG_ANG_AAG	R	ANGAJATI_AAG
UNQ_NUME_PREN_AAG	U	ANGAJATI_AAG
UNQ_EMAIL_AAG	U	ANGAJATI_AAG
PK_ANG_AAG	P	ANGAJATI_AAG
...
create  table ANGAJATI_AAG(
  ...
  cod_dep number(2) ,
  comision number(4,2),
  constraint unq_nume_pren_AAG unique(nume, prenume),
  constraint ck_sal_com_AAG check(salariu>comision *100),
  constraint pk_ang_AAG primary key(cod_ang),
  constraint unq_email_AAG unique(email),
  constraint fk_ang_ang_AAG foreign key(cod_sef) references angajati_AAG(cod_ang),
  constraint ck_cod_dep_AAG check(cod_dep>0),
  constraint fk_ang_depart_AAG foreign key(cod_dep) references departamente_AAG(cod_dep)  
    );
    
    drop table ANGAJATI_AAG;

  SELECT * FROM USER_CONSTRAINTS WHERE lower(TABLE_NAME) = 'angajati_aag';
...
insert into angajati_AAG (Cod_ang, Nume, Prenume, Job, Cod_sef, Salariu, Cod_dep)
...
insert into angajati_AAG  --inserare prin copiere din tabela mea
select * from angajati_AAG;
-----
insert into angajati_AAG(Cod_ang, Nume ,Prenume, Email, Data_ang ,Job, Cod_sef, Salariu, Cod_dep)
values( 100 ,'Nume1', 'Prenume1', Null ,Null, 'Director', null, 20000 ,10);
...
insert  into angajati_AAG
values (105, 'Nume6', 'Prenume6', 'Nume6@gmail.com', Null, 'Analist', 101, 3000, 50, 0.2);
SQL Error: ORA-02291: constrangere de integritate (GRUPA33.FK_ANG_DEPART_AAG) violata - cheia parinte negasita
--dept 50 nu exista in lista de departamente

--atentie
insert into angajati_AAG
values (105, 'Nume5', 'Prenume5', 'Nume5@gmail.com', Null, 'Analist', 101, 3000, 30, 0.2);
--SQL Error: ORA-00001: restrictia unica (GRUPA34.UNQ_NUME_PREN_AAG) nu este respectata

insert  into angajati_AAG
values (105, 'Nume6', 'Prenume5', 'Nume5@gmail.com', Null, 'Analist', 101, 3000, 30, 0.2);
--SQL Error: ORA-00001: restrictia unica (GRUPA34.UNQ_EMAIL_AAG) nu este respectata
...
--SQL Error: ORA-02291: constrangere de integritate (GRUPA33.FK_ANG_ANG_AAG) violata - cheia parinte negasita
...
truncate table departamente_AAG; 
--SQL Error: ORA-02266: cheile unice/primare din tabela sunt referite de cheile externe activate
delete from departamente_AAG; 
--SQL Error: ORA-02292: constrangerea de integritate (GRUPA33.fk_ang_depart_AAG) violata - gasita inregistrarea copil
...
SELECT constraint_name, constraint_type, table_name
FROM     user_constraints
WHERE  lower(table_name) IN ('departamente_AAG', 'angajati_AAG');

--sterg vechea constrangere FK dintre tabela angajati si cea de departamente
alter table angajati_AAG
drop constraint FK_ANG_DEPART_AAG;
--table ANGAJATI_AAG altered.


--adaug FK intre tabela angajati si cea de departamente
alter table angajati_AAG
add constraint FK_ANG_DEPART2_AAG foreign key(cod_dep) 
            references departamente_AAG(cod_dep) on delete cascade;
...
rollback;
select * from angajati_AAG; --7 rez
select * from departamente_AAG; --3 rez
--commit;

-- ON DELETE SET NULL

alter table angajati_AAG
drop constraint FK_ANG_DEPART2_AAG ;
--table ANGAJATI_AAG altered.
...
drop sequence   seq_dept_AAG;

create  sequence seq_dept_AAG
start with 400
increment by 10
maxvalue 10000
nocycle
nocache;
--Sequence SEQ_DEPT_AAG created.

select seq_dept_AAG.nextval
from dual;

--I -> 400
--II ->410

select seq_dept_AAG.currval
from dual;

insert into departamente_AAG
values(seq_dept_AAG.nextval, 'Dept_sec', null);  --420
...
--create table dept_AAG
--as select * from departments;

insert into dept_AAG
values(seq_dept_AAG.nextval, 'Dept_sec', null, null); 
--1 rows inserted.
select * from dept_AAG;
--430	Dept_sec		null null
--440	Dept_sec		null null 

delete from dept_AAG
...
drop sequence seq_ang_AAG;

create sequence seq_ang_AAG
start with 100
maxvalue 120 --- ca sa apara o eroare cand ajunge la maxvalue
nocycle
nocache;

41. Sa se modifice toate liniile din angajati_AAG (daca nu mai exista, il recreati),
...
--SQL Error: ORA-02292: constrangerea de integritate (GRUPA34.FK_ANG_ANG_AAG) violata - gasita inregistrarea copil
...
update angajati_AAG --6 ang
set cod_ang = seq_ang_AAG.nextval; --(107---112)
--SQL Error: ORA-02292: constrangerea de integritate (GRUPA34.FK_ANG_ANG_AAG) violata - gasita inregistrarea copil
...
update angajati_AAG
set cod_ang = seq_ang_AAG.nextval;
6 rows updated.
...
ORA-08004: sequence SEQ_ANG_AAG.NEXTVAL exceeds MAXVALUE and cannot be instantiated
