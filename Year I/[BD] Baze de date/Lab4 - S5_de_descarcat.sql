...
1. S? se creeze tabelele EMP_AAG, DEPT_AAG (?n ?irul de caractere ?ong?, p reprezint? 
...
CREATE TABLE emp_AAG AS SELECT * FROM employees;
CREATE TABLE DEPT_AAG AS SELECT * FROM departments; --table DEPT_AAG created.

select * from DEPT_AAG; --27 rez
select * from emp_AAG;
...
--commit;
drop table emp_AAG; --sterge tabelul emp_AAG
drop table DEPT_AAG;
...
--
--drop table DEPT_AAG;
--CREATE TABLE DEPT_AAG AS SELECT * FROM departments;
...
delete from dept_AAG where department_id in (111,112);
...
ALTER TABLE emp_AAG
ADD CONSTRAINT pk_emp_AAG PRIMARY KEY(employee_id);
    --- pk_emp_AAG = nume unic de constrangere
    --punem cheia primara pe coloana employee_id a tabelei emp_AAG
--table emp_AAG altered.
...
--adaugam cheia primara pe coloana department_id din tabela de DEPT_AAG
ALTER TABLE DEPT_AAG 
ADD CONSTRAINT pk_DEPT_AAG PRIMARY KEY(department_id); 
--table DEPT_AAG altered.
...
ALTER TABLE emp_AAG
ADD CONSTRAINT fk_emp_DEPT_AAG FOREIGN KEY(department_id)   --dept_id din emp_AAG
                        REFERENCES DEPT_AAG(department_id);
--fiecare angajat lucreaza pe un departament care exista in tabela de departamente                                         
--table emp_AAG altered.


----Atentie, daca nu este setata cheia primara pe tabela dept_AAG, atunci nu vom putea
--seta cheia externa pe tabela emp_AAG
...
--Suplimentar:
alter table emp_AAG
drop constraint FK_EMP_DEPT_AAG;

alter table DEPT_AAG
drop constraint PK_DEPT_AAG;
...
5. S? se insereze departamentul 300, cu numele Programare ?n DEPT_AAG.
...
select * from DEPT_AAG where department_id = 300;
rollback;

commit;

delete from DEPT_AAG where department_id =300;

delete from DEPT_AAG where department_id in (111,112);
b)
INSERT INTO DEPT_AAG (department_id, department_name)
VALUES (300, 'Programare');
--1 rows inserted.
select * from DEPT_AAG where department_id =300;
...
--INSERT INTO DEPT_AAG (department_id, department_name)
--incerc sa mai rulez inca odata insert-ul => eroare: ORA-00001: restrictia unica (GRUPA33.PK_DEPT_AAG) nu este respectata
...
c) 
INSERT INTO DEPT_AAG (department_name, department_id)
VALUES (300, 'Programare'); 
...
select * from DEPT_AAG where department_id = 111;
commit;
rollback;

delete from dept_AAG where department_id=111;

d)
INSERT INTO DEPT_AAG (department_id, department_name, location_id)
...
rollback;
e) 
INSERT INTO DEPT_AAG (department_name, location_id)
VALUES ('Programare', null);
--ORA-01400: nu poate fi inserat NULL ?n ("GRUPA33"."DEPT_AAG"."DEPARTMENT_ID")
...
--In final, vom adauga dept 300 in tabela DEPT_AAG
INSERT INTO DEPT_AAG (department_id, department_name)
VALUES (300, 'Programare');
...
6. S? se insereze un angajat corespunz?tor departamentului introdus anterior ?n tabelul emp_AAG,
...
--delete from emp_AAG where employee_id =252;
insert into emp_AAG
...
--ORA-01400: cannot insert NULL into ("GRUPA32"."EMP_AAG"."LAST_NAME")

select * from emp_AAG; --108 ang
commit;
...
insert into emp_AAG (employee_id, last_name, email, hire_date, job_id, department_id)
...
INSERT INTO emp_AAG (employee_id, last_name, email, hire_date,
...
----Am facut Insert 252 + commit + Inseram 253, 254 + commit + rollback 
      -- => In emp_AAG se gasesc toti cei 3 ang: 252, 253, 254

--atentie la departament
insert into emp_AAG (employee_id, last_name, email,  hire_date, job_id,
                        department_id)
values (255, 'nume', 'nume@gmail.com', sysdate, 'test_job', 600); 
--ORA-02291: constr?ngere de integritate (GRUPA33.FK_EMP_DEPT_AAG) violata - cheia parinte negasita
...
10. Crea?i un nou tabel, numit EMP1_AAG, care va avea aceea?i structur? ca ?i EMPLOYEES, 
dar nicio ?nregistrare. 
Copia?i ?n tabelul EMP1_AAG salaria?ii (din tabelul EMPLOYEES) 
al c?ror comision dep?e?te 25% din salariu. 
 
--V1: Crea?i un nou tabel, numit EMP1_AAG, care va avea aceea?i structur? ca ?i EMPLOYEES, dar nicio ?nregistrare. 
CREATE TABLE emp1_AAG AS SELECT * FROM employees WHERE 1=0;  --comanda are commit implicit

select * from emp1_AAG; --0 rez

--stergem tabelul emp1
drop table emp1_AAG;

--V2: Crea?i un nou tabel, numit EMP1_AAG, care va avea aceea?i structur? ca ?i EMPLOYEES, dar nicio ?nregistrare. 
CREATE TABLE emp1_AAG AS SELECT * FROM employees; --commit implicit
select * from emp1_AAG; --107 rez
DELETE FROM emp1_AAG; --necesar daca nu aveam clauza WHERE de mai sus
--107 rows deleted.
select * from emp1_AAG; --0 rez
...
desc emp1_AAG;
--incorect
INSERT INTO emp1_AAG (last_name, email,  hire_date, job_id)
SELECT *  --aici este pb
...
INSERT INTO emp1_AAG (employee_id, last_name, email,  hire_date, job_id, department_id)
...
alter table emp1_AAG
add constraint pk_emp1 primary key(employee_id);

SELECT employee_id, last_name, salary, commission_pct FROM emp1_AAG; --33 rez

Ce va con?ine tabelul EMP1_AAG ?n urma acestei succesiuni de comenzi?
ROLLBACK;

SELECT employee_id, last_name, salary, commission_pct FROM emp1_AAG; 
...
--Table EMP1_AAG dropped.



---atentie
CREATE TABLE emp1_AAG AS SELECT * FROM employees WHERE commission_pct >0.25 ; 
...
-- daca datele nu sunt corecte in tabelul emp_AAG( le suprascriu cu cele din employees, atentie la 252, 253, 254)
update emp_AAG e
...
15. M?ri?i salariul tuturor angaja?ilor din tabelul EMP_AAG cu 5%. 
...
20. ?terge?i toate ?nregistr?rile din tabelul DEPT_AAG. Ce ?nregistr?ri se pot ?terge? Anula?i modific?rile.

delete from DEPT_AAG;
--SQL Error: ORA-02292: constr?ngerea de integritate (GRUPA33.FK_EMP_DEPT_AAG) violata - gasita ?nregistrarea copil
...
select department_id
 ...
 where department_id in ( select department_id
  ...
  select * from DEPT_AAG; --12 rez  
--90	Executive	100	1700
--100	Finance	108	1700
--110	Accounting	205	1700
--300	Programare		
  rollback;
 --28 de dept in DEPT_AAG
...
delete from DEPT_AAG
 where department_id not in (select department_id 
                           from employees
                           where department_id is not null);
--ORA-02292: integrity constraint (GRUPA32.FK_EMP_DEPT_AAG) violated - child record found                           
...
23. S? se ?tearg? un angajat din tabelul EMP_AAG. Modific?rile vor deveni permanente.
...
select * from emp_AAG; --109 ang

--select count(*)
--from emp_AAG; --109 ang
...
delete from emp_AAG; --110 rows deleted.
...
select * from emp_AAG; --110 ang : 252, 253, 255 
 commit;
 
 
 --Ex1: Sa se afiseze orasul si codul locatiei in care lucreaza in prezent salariatii
 --angajati pe un job al carui titlu se termina in "clerk" sau care se gaseste intr-o tara a carei denumire are
 --pe a 4-a sau a 5-a pozitie litera "a".
SELECT l.city, l.location_id, j.job_title, c.country_name
FROM jobs j join employees e on j.job_id = e.job_id
      join departments d on e.department_id = d.department_id 
      join locations l on d.location_id = l.location_id
      join countries c on l.country_id = c.country_id
WHERE lower(j.job_title) like '%clerk'    
or (lower(c.country_name) like '___a%' or lower(c.country_name) like '____a%');
 
--Ex 2a): Sa se afiseze numele complet, job-ul (titlul) actual si departamentul (numele) actual
...
--Se va tine cont atat de job, cat si de departament. Ordonati rezultatele descrescator in functie de codul departamentului si  apoi dupa titlul jobului.
 
SELECT e.last_name, e.first_name, j.job_title, d.department_name
FROM jobs j join employees e on j.job_id = e.job_id
     join departments d on e.department_id = d.department_id
WHERE e.department_id is not null
and j.job_id in (SELECT job_id
                 FROM job_history
                 WHERE start_date > '1993-10-29');
